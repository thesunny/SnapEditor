define ["jquery.custom", "core/browser", "core/helpers"], ($, Browser, Helpers) ->
  window.SnapEditor.internalPlugins.list =
    events:
      activate: (e) ->
        $(e.api.el).on("keydown", e.api.config.plugins.list.handleTab)
      deactivate: (e) ->
        $(e.api.el).off("keydown", e.api.config.plugins.list.handleTab)
    commands:
      orderedList: Helpers.createCommand("orderedList", "ctrl.shift.8", (e) -> e.api.config.plugins.list.insert(e))
      unorderedList: Helpers.createCommand("unorderedList", "ctrl.shift.7", (e) -> e.api.config.plugins.list.insert(e))
      indent: Helpers.createCommand("indent", "", (e) -> e.api.config.plugins.list.dent(e))
      outdent: Helpers.createCommand("outdent", "", (e) -> e.api.config.plugins.list.dent(e))
